...
class YuME_tileImporter : EditorWindow
{
            ...
            if (child.GetComponent<MeshFilter>() != null)
            {
                if (child.GetComponent<MeshFilter>().sharedMesh != null)
                {
                    ...
                    if (!(child.name.Contains(userSettings.collisionIdentifier) || child.name.Contains(userSettings.customBoundsIdentifier)))
                    {
                        ...
                        tileMeshFilter.sharedMesh = recenterMesh(child.GetComponent<MeshFilter>().sharedMesh, true);
                        tileMeshDataObject.transform.position = setTheTileMeshObjectOffsetFromTheParent(tileMeshFilter.sharedMesh.bounds.extents);

                        attachCustomCollisionMesh(child); // check if the object has a custom collision mesh and perform setup
    ...
    static void attachCustomCollisionMesh(Transform child)
    {
        ...
        if (!userSettings.bypassCollisionSetup && userSettings.importCollision)
        {
			...
			foreach (Transform collisionMesh in userSettings.sourceTiles.transform) // search the source file for the associated collision mesh based the identifer setup in the tool
            {
                ...
                {
                    tileMeshCollider.sharedMesh = collisionMesh.GetComponent<MeshFilter>().sharedMesh;
                    tileMeshCollider.sharedMesh.bounds = findCustomBoundsCollision(child.GetComponent<MeshFilter>().sharedMesh, tileMeshCollider.sharedMesh);
                    tileMeshCollider.sharedMesh = recenterMesh(collisionMesh.GetComponent<MeshFilter>().sharedMesh, true); // recenter the collision mesh so it matches the source mesh
                    return;
                }
            }
        }
    ...
    static Bounds findCustomBoundsCollision(Mesh sourceMesh, Mesh collisionMesh)
    {
        Bounds customBounds = collisionMesh.bounds;
        ...
        if (userSettings.importCustomBounds)
        {
            foreach (Transform boundsMesh in userSettings.sourceTiles.transform) // search the source file for the associated collision mesh based the identifer setup in the tool
            {
                if (boundsMesh.name == tileParentObject.name + userSettings.customBoundsIdentifier) // check to see if we found the collision mesh
                {
                    if (boundsMesh.GetComponent<MeshFilter>().sharedMesh != null && boundsMesh.GetComponent<MeshFilter>().sharedMesh.bounds != collisionMesh.bounds)
                    {
                        Debug.Log("Custom Bounds for Collision Found");
                        customBounds = boundsMesh.GetComponent<MeshFilter>().sharedMesh.bounds;
                    }
                }
            }
    ...
    static Mesh recenterMesh(Mesh sourceMesh, bool createAsset)
    {
