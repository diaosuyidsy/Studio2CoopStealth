...
public class DetectHit : MonoBehaviour
{
	...
	{
		Vector3 v = collision.gameObject.GetComponent<Collider>().ClosestPointOnBounds(transform.position);
		...
		if (forceField.mesh != null)
		{
			...
			foreach (MeshRenderer tMesh in forceField.mesh)
			{
				tMesh.materials[forceField.i].SetVector("_Position", transform.InverseTransformPoint(v));
			}
			forceField.mTime = 1.0f;
			forceField.hit = true;
			forceField.hit2 = true;
