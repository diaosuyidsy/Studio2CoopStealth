...
namespace DestroyIt
{
    public class ReleaseObject : MonoBehaviour
    {
        ...
        void Start()
        {
            velocityLastUpdate = GetComponent<Rigidbody>().velocity;
        }

        void FixedUpdate()
        {
            Vector3 velocityChange = (GetComponent<Rigidbody>().velocity - velocityLastUpdate) / GetComponent<Rigidbody>().mass;
