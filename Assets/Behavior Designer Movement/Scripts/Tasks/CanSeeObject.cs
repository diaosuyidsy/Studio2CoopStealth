	...
	public class CanSeeObject : Conditional
	{
		...
		public override TaskStatus OnUpdate()
		{
			...
			else
			{
				...
				{ // If the target is not null then determine if that object is within sight
					ro = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value, viewDistance.Value, targetObject.Value, targetOffset.Value, ignoreLayerMask, useTargetBone.Value, targetBone);
				}
				else if (!string.IsNullOrEmpty(targetTag.Value))
				{ // If the target tag is not null then determine if there are any objects within sight based on the tag
					ro = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value, viewDistance.Value, GameObject.FindGameObjectWithTag(targetTag.Value), targetOffset.Value, ignoreLayerMask, useTargetBone.Value, targetBone);
				}
				else
				{ // If the target object is null and there is no tag then determine if there are any objects within sight based on the layer mask
					...
					ro = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value, viewDistance.Value, overlapColliders, objectLayerMask, targetOffset.Value, ignoreLayerMask, useTargetBone.Value, targetBone);
				}
			}
